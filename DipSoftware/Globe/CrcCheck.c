...
const uint16 CRCR16_8408_Table[16] = {
	  0x0000,0x1081,0x2102,0x3183,0x4204,0x5285,0x6306,0x7387,0x8408,0x9489,0xA50A,0xB58B,0xC60C,0xD68D,0xE70E,0xF78F
};

uint16 GetCRCR16_8408_FFFF_FFFF_Table(unsigned char *crcBuff, unsigned char crcLen) {
		uint8 i;
		uint16 crc = 0xFFFF;
		uint8 crcTemp;
		for(i = 0; i < (crcLen * 2); i ++) {
				...
				crc = (crc >> 4) ^ CRCR16_8408_Table[(crc & 0x0F) ^ crcTemp];
		}
		crc ^= 0xFFFF;
		return crc;
}
